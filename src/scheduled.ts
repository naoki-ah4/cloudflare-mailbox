...
export const handleScheduled = async (
  ...
  try {
    ...
    if (hour === "2") {
      ...
      await ctx.waitUntil(performDailyBackup(env));
    }
    else if (hour === "3") {
      // 深夜3時台: 週次バックアップ
      await ctx.waitUntil(performWeeklyBackup(env));
    }
    else if (hour === "4") {
      // 深夜4時台: 月次バックアップ
      await ctx.waitUntil(performMonthlyBackup(env));
    }
    
    logger.info("スケジュールタスク完了", { cron });
  } catch (error) {
...
const performDailyBackup = async (env: Env): Promise<void> => {
  try {
    ...
    logger.info("日次バックアップ完了", { backupKey });
  } catch (error) {
...
const performWeeklyBackup = async (env: Env): Promise<void> => {
  try {
    ...
    logger.info("週次バックアップ完了", { backupKey });
  } catch (error) {
...
const performMonthlyBackup = async (env: Env): Promise<void> => {
  try {
    ...
    logger.info("月次バックアップ完了", { backupKey });
  } catch (error) {
