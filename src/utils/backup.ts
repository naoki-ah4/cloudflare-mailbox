...
export const createFullBackup = async (
  ...
  try {
    logger.info("フルバックアップ開始", { context: { backupType } });

    // 各KVストアからデータを取得
    const [usersData, messagesData, mailboxesData, systemData] =
      await Promise.all([
...
const exportKVData = async (
  ...
  try {
    ...
    logger.error(`${storeName}データエクスポートエラー`, {
      error: error as Error,
      storeName,
    });
    throw error;
...
const importKVData = async (
  ...
  try {
    ...
    logger.error(`${storeName}データインポートエラー`, {
      error: error as Error,
      storeName,
      importedKeys,
    });
...
export const listBackups = async (
  env: Env,
  prefix = "backups/"
): Promise<Array<{ key: string; metadata: BackupMetadata | null }>> => {
  try {
        ...
        if (obj.customMetadata) {
          metadata = {
            ...
            compressedSize: parseInt(
              obj.customMetadata["compressed-size"] || "0"
            ),
            backupType:
              (obj.customMetadata[
                "backup-type"
              ] as BackupMetadata["backupType"]) || "manual",
          };
        }
