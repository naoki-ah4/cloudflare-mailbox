...
export const SystemKV = {
  ...
  async addHistoryEntry(kv: KVNamespace, entry: SystemSettingsHistoryEntry): Promise<void> {
  ...
  async updateSettings(kv: KVNamespace, allowedDomains: string[], adminId: string, changes?: string): Promise<SystemSettings> {
    ...
    const historyEntry: SystemSettingsHistoryEntry = {
      ...
      updatedAt: now,
      updatedBy: adminId,
      changes: changes || (existingSettings 
