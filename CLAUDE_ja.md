...
- **保存先**: R2 Storage
- **自動実行**: Cron Triggers
  - 毎日深夜2時: 日次バックアップ + 古いファイル削除
  - 毎週日曜深夜3時: 週次バックアップ
  - 毎月1日深夜4時: 月次バックアップ
- **対象データ**: USERS_KV、MESSAGES_KV、MAILBOXES_KV、SYSTEM_KV
- **除外データ**: セッション（`session:`）、レートリミット（`rate_limit:`）

#### 世代管理

- **日次バックアップ**: 7日間保持
- **週次バックアップ**: 4週間保持（日次から昇格）
- **月次バックアップ**: 1年間保持（週次から昇格）
- **手動バックアップ**: 30日間保持
- **自動間引き**: 毎日のクリーンアップで古いファイルを削除

#### 管理機能

- **バックアップ一覧**: 作成日時、種別、サイズ、圧縮率表示
- **手動バックアップ**: ワンクリック作成
- **復旧機能**: バックアップからの完全復元
- **削除機能**: 個別ファイル削除
- **統計表示**: 種別別集計、保持期間、総サイズ
...
最終更新: 2025 年 7 月 3 日（バックアップシステム仕様追加）
